	...
	) 
{
	if(ppcloud.size() <= 7 || imgPoints.size() <= 7 || ppcloud.size() != imgPoints.size()) { 
        ...
        cerr << "couldn't find [enough] corresponding cloud points... (only " << ppcloud.size() << ") #:" << working_view <<endl;
		return false;
	}
	...
	if(inliers.size()==0) { //get inliers
        for(uint i=0;i<projected3D.size();i++) {
...
//	}
    ...
    for(uint i=0;i<new_triangulated.size();i++) { reprj_errors.push_back(new_triangulated[i].reprojection_error); }
    ...
    for(uint i=0;i<new_triangulated.size();i++) {
		if(trig_status[i] == 0)
    ...
    for (uint j = 0; j<new_triangulated.size(); j++) {
...
//#pragma omp parallel for
    for (uint _i=0; _i < imgs.size() - 1; _i++)
	{
...
void MultiCameraPnP::RecoverDepthFromImages() {
	...
	while (done_views.size() != imgs.size())
	{
		...
		for (set<int>::iterator done_view = good_views.begin(); done_view != good_views.end(); ++done_view) 
		{
			...
			std::cout << "before triangulation: " << pcloud.size();
            for (uint j=0; j<add_to_cloud.size(); j++) {
				if(add_to_cloud[j] == 1)
					pcloud.push_back(new_triangulated[j]);
			}
			std::cout << " after " << pcloud.size() << std::endl;
